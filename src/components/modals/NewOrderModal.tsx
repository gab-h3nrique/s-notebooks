...
const NewOrderModal = ({isOpen, onClose, id, orderHandle}:Props) => {
    ...
    async function sendEmail() {
        ...
        try {
            ...
            notification.push({ type: 'success', title: 'Sucesso!', description: 'O email foi enviado com êxito.', time: 2000 })
                                                ...
                                                services.length > 0 && services?.map((item, i)=>{
                                                    return (
                                                        <div key={i} onClick={() => openModalService(item, i)} className="grid grid-cols-12 gap-2 cursor-pointer">
                                                            ...
                                                            <input disabled value={valueMask(item.value)} className="pointer-events-none cursor-pointer col-span-2 text-sm font-medium text-slate-600 rounded-lg w-full bg-gray-50 p-1 border-2 border-gray-300 outline-none focus:border-transparent focus:ring focus:ring-orange-400 hover:scale-y-105 duration-150 opacity-80" />
